...
COLUMN_CONFIGS = [
    ("submitted_at", "submitted_at_fmt", "Convert submitted_at to ISO format"),
    ("processed_on", "processed_on_fmt", "Convert processed_on to DD-MM-YYYY"),
    ("closed_at", "closed_at_fmt", "Convert closed_at to YYYY-MM-DD HH:mm:ss"),
]
...
def generate_dates(count: int) -> list[str]:
    ...
    for _ in range(count):
        rand_days = random.randint(0, 1400)
